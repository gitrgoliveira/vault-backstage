# L0 — Vault Tenant Onboarding (Admin)
...
- Per-environment HCP Terraform projects (e.g. `acme-Vault-dev`, `acme-Vault-test`, `acme-Vault-prod`)
...
|-------|----------|------|-------------|
| **Workspace Name** | Yes | `string` | Unique name for the HCP TF workspace. Must match `^[A-Za-z0-9_-]+$`. |
| **Tenant** | Yes | `string` | Tenant identifier (2–32 chars, alphanumeric and hyphens). Used in project and namespace naming. |
| **Admin Project Name** | No | `string` | The HCP TF admin project carrying the Vault credentials variable set. Defaults to `HCP Vault Admin`. |
