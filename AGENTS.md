...
- [`plugins/hcp-terraform-backend/`](plugins/hcp-terraform-backend) — the core backend plugin (`@internal/plugin-hcp-terraform-backend`). `HcpTfClient.ts` (HCP TF JSON:API client), `HcpTerraformWorkspaceProvider.ts` (catalog EntityProvider), `actions/` (scaffolder actions), `module.ts` (registration), `config.d.ts` (typed `hcpTerraform` config).
- [`plugins/vault-frontend/`](plugins/vault-frontend) — frontend plugin (`@internal/plugin-vault-frontend`). Contains the catalog entity cards (`VaultWorkspaceOutputsCard`, `VaultNextLayerCard`) registered via `EntityCardBlueprint`, the sidebar filter (`VaultLayerPicker` + `VaultLayerFilter`) registered via `CatalogFilterBlueprint`, and the `ScopedEntityPicker` custom scaffolder field registered via `FormFieldBlueprint`. Exports `vaultCatalogModule` and `vaultScaffolderModule` for one-line registration in `App.tsx`.
