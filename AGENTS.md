...
- [`plugins/hcp-terraform-backend/`](plugins/hcp-terraform-backend) — the core backend plugin (`@gitrgoliveira/plugin-hcp-terraform-backend`). `HcpTfClient.ts` (HCP TF JSON:API client), `HcpTerraformWorkspaceProvider.ts` (catalog EntityProvider), `actions/` (scaffolder actions), `module.ts` (registration), `config.d.ts` (typed `hcpTerraform` config).
- [`plugins/vault-frontend/`](plugins/vault-frontend) — frontend plugin (`@gitrgoliveira/plugin-vault-frontend`). Contains the catalog entity cards (`VaultWorkspaceOutputsCard`, `VaultNextLayerCard`) registered via `EntityCardBlueprint`, the sidebar filter (`VaultLayerPicker` + `VaultLayerFilter`) registered via `CatalogFilterBlueprint`, and the `ScopedEntityPicker` custom scaffolder field registered via `FormFieldBlueprint`. Exports `vaultCatalogModule` and `vaultScaffolderModule` for one-line registration in `App.tsx`.
...
- **Parent-workspace pickers use `ui:field: ScopedEntityPicker`** (custom field in `plugins/vault-frontend/src/components/ScopedEntityPicker.tsx`, registered on `pluginId: scaffolder` via `vaultScaffolderModule`). It behaves like `EntityPicker` but filters candidates to the tenant/env selected in a sibling field via `ui:options.scopeField: onboardedTarget` (matching the workspace's `hcptf.io/target` annotation), so a use-case can't be linked to a parent in a different tenant. The `onboardedTarget` picker itself stays a plain `EntityPicker` (it is the scope source). It reads siblings through `props.formContext.formData`.
